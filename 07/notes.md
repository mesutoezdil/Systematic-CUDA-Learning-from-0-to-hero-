...
A GPU is only fast if it can get data fast enough.  
...
When you combine all of this, one thing becomes clear:

GPU performance is not just about cores.  
...
The first one has 100 cores.  
The second one has 200 cores.
...
Having more cores is not enough.
...
So performance depends on two things working together:

how many cores we have, and how fast each core runs.
...
A better way to think about performance is this:

How much work can this GPU finish in a given time?
...
And this is the key idea to remember:

GPU performance is not a single number.
